...
namespace BackendFramework.Repositories
{
    ...
    public class UserEditRepository : IUserEditRepository
    {
        ...
        public async Task<List<UserEdit>> GetAllEntries(string projectId)
        {
        ...
        /// <summary> Finds <see cref="UserEdit"/> with specified entryId and projectId </summary>
        public async Task<UserEdit?> GetEntry(string projectId, string entryId)
        {
        ...
        public async Task<UserEdit> Create(UserEdit entry)
        {
            await _userEditDatabase.UserEdits.InsertOneAsync(entry);
            return entry;
        }

        /// <summary> Removes <see cref="UserEdit"/> with specified entryId and projectId </summary>
        /// <returns> A bool: success of operation </returns>
        ...
        /// <summary> Replaces <see cref="UserEdit"/> with specified entryId and projectId </summary>
        /// <returns> A bool: success of operation </returns>
        public async Task<bool> Replace(string projectId, string entryId, UserEdit entry)
        {
            var filterDef = new FilterDefinitionBuilder<UserEdit>();
            var filter = filterDef.And(filterDef.Eq(
                x => x.ProjectId, projectId), filterDef.Eq(x => x.Id, entryId));

            var result = await _userEditDatabase.UserEdits.ReplaceOneAsync(filter, entry);
