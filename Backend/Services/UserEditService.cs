...
namespace BackendFramework.Services
{
    ...
    public class UserEditService : IUserEditService
    {
        ...
        public UserEditService(IUserEditRepository editRepo)
        {
            _editRepo = editRepo;
        }
        ...
        public async Task<Tuple<bool, int>> AddGoalToUserEdit(string projectId, string userEditId, Edit edit)
        {
            ...
            var oldUserEdit = await _editRepo.GetUserEdit(projectId, userEditId);
            ...
            var replaceSucceeded = await _editRepo.Replace(projectId, userEditId, newUserEdit);
            if (!replaceSucceeded)
            {
        ...
        public async Task<bool> AddStepToGoal(string projectId, string userEditId, int goalIndex, string newStep)
        {
            ...
            var updateResult = await _editRepo.Replace(projectId, userEditId, newUserEdit);
            return updateResult;
        }
