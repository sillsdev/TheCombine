...
describe("Tests DataEntryTable", () => {
  ...
  it("should call add word on backend when new entry has data and complete is clicked", () => {
    mockAxiosPost.mockClear();
  ...
  it("should NOT call add word on backend when new entry has no data and complete is clicked", () => {
    mockAxiosPost.mockClear();
    // Verify that NewEntry is present
    let newEntryItems = testRenderer.root.findAllByType(NewEntry);
    expect(newEntryItems.length).toBe(1);
