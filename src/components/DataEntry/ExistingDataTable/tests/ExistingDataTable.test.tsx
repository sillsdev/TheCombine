...
describe("Tests ExistingData", () => {
  ...
  it("filters words by domain", () => {
    ...
    expect(filterWordsByDomain(unfilteredWords, domains[0]).length).toBe(expectedLength);
  });
  ...
  it("sorts words alphabetically", () => {
    ...
    let ExistingDataTableItems = testRenderer.root.findAllByType(ExistingDataTable);
    expect(ExistingDataTableItems.length).toBe(1);
    var ExistingDataTableHandle: ReactTestInstance = ExistingDataTableItems[0];

    ExistingDataTableHandle.instance.setState({existingWords: unfilteredWords}, () => 
        {});//TODO
  });
});
