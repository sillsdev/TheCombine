...
export const WordApiAxiosParamCreator = function (
  ...
  return {
     ...
     * @throws {RequiredError}
     */
    doesProjectHaveDefinitions: async (
      projectId: string,
      options: any = {}
    ): Promise<RequestArgs> => {
      // verify required parameter 'projectId' is not null or undefined
      assertParamExists("doesProjectHaveDefinitions", "projectId", projectId);
      const localVarPath =
        `/v1/projects/{projectId}/words/doesprojecthavedefinitions`.replace(
          `{${"projectId"}}`,
          encodeURIComponent(String(projectId))
        );
      // use dummy base URL string because the URL constructor only accepts absolute URLs.
...
export const WordApiFp = function (configuration?: Configuration) {
  ...
  return {
     ...
     * @throws {RequiredError}
     */
    async doesProjectHaveDefinitions(
      projectId: string,
      options?: any
    ): Promise<
      (axios?: AxiosInstance, basePath?: string) => AxiosPromise<boolean>
    > {
      const localVarAxiosArgs =
        await localVarAxiosParamCreator.doesProjectHaveDefinitions(
          projectId,
          options
        );
      return createRequestFunction(
...
export const WordApiFactory = function (
  ...
  return {
    ...
    doesProjectHaveDefinitions(
      ...
      return localVarFp
        .doesProjectHaveDefinitions(projectId, options)
        .then((request) => request(axios, basePath));
    },
    /**
     *
...
/**
 * Request parameters for doesProjectHaveDefinitions operation in WordApi.
 * @export
 * @interface WordApiDoesProjectHaveDefinitionsRequest
 */
export interface WordApiDoesProjectHaveDefinitionsRequest {
  /**
   *
   * @type {string}
   * @memberof WordApiDoesProjectHaveDefinitions
   */
  readonly projectId: string;
}
...
export class WordApi extends BaseAPI {
  ...
  /**
   *
   * @param {WordApiDoesProjectHaveDefinitionsRequest} requestParameters Request parameters.
   * @param {*} [options] Override http request option.
   * @throws {RequiredError}
   * @memberof WordApi
   */
  public doesProjectHaveDefinitions(
    requestParameters: WordApiDoesProjectHaveDefinitionsRequest,
    options?: any
  ) {
    return WordApiFp(this.configuration)
      .doesProjectHaveDefinitions(requestParameters.projectId, options)
      .then((request) => request(this.axios, this.basePath));
  }
