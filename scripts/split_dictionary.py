...
def main() -> None:
    ...
    header_line += f" -l {args.lang} -t {args.threshold} -T {args.Threshold}"
    if args.max:
        header_line += f" -m {args.max}"
    header_line += "`.\n\n"

    # Generate the exported array of keys for this language's dictionary parts...
    key_lines: List[str] = ["export const keys = [\n"]
    # ... and the switch cases for fetching dictionary parts by key
    switch_lines: List[str] = ["  switch (key) {\n"]
    ...
    with open(index_file_path, "w") as index_file:
        index_file.write(header_line)
        index_file.writelines(key_lines)
        index_file.writelines(default_function_lines)
