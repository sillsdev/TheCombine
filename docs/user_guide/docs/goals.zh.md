...
*创建字符库存*提供了该项目词条土语形式出现的每个符编码标准字符的概览。 这会使用户识别出该语言中经常用到的字符，并将它们“接受”为该语言字符库存的一部分。 字符库属于项目土语的LDML文档一部分，在项目[导出](project.md#export)时会包含该文件。 接受字符后，就能用统一的字符编码标准、在民族语及其他语言资源中表达。
